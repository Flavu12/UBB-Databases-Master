...
# XGBoost optional (daca e instalat)
try:
...
# In dataset-ul Pima, 0 la aceste coloane inseamna de multe ori "lipsa"
...
def fit_preprocess(X_train: pd.DataFrame):
    """Fit pe train: imputare + standardizare (evitam leakage)."""
...
def transform_preprocess(imputer, scaler, X: pd.DataFrame):
    """Transform pe orice set nou folosind obiectele fit-uite pe train."""
...
def get_model(model_name: str):
    ...
    if model_name == "xgb":
        if not HAS_XGBOOST:
            raise RuntimeError("XGBoost nu este instalat. Ruleaza: pip install xgboost")
...
def evaluate_one_model(csv_path: str, model_name: str, test_size: float = 0.2, seed: int = 42, save_cm: bool = True):
    ...
    if save_cm:
        ...
        # presupune ca plot_confusion_matrix salveaza (nu plt.show)
        try:
            plot_confusion_matrix(cm, pretty_name, save_path=cm_path)
        except TypeError:
            # daca functia ta nu are save_path in semnatura, o chemam simplu
...
def read_person_from_keyboard() -> dict:
    ...
    pregnancies = read_float("• Număr sarcini: ")
    glucose = read_float("• Nivel glucoză: ")
    bloodpressure = read_float("• Tensiune arterială: ")
    skinthickness = read_float("• Grosime pliului pielii: ")
    insulin = read_float("• Nivel insulină (0 dacă nu se cunoaște): ")
    bmi = read_float("• BMI: ")
    dpf = read_float("• Diabetes Pedigree Function: ")
    age = read_float("• Vârstă: ")
...
def predict_person_interactive(csv_path: str, model_name: str):
    ...
    if int(prediction) == 1:
        print("\n🔴 Verdict: PACIENTUL ARE DIABET (Outcome=1)")
    else:
        print("\n🟢 Verdict: PACIENTUL NU ARE DIABET (Outcome=0)")
